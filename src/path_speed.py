    ...
    def find_path( self ):
        ...
        while( not rospy.is_shutdown() ):
            ...
            if( count == 2 ):
                ...
                while( not rospy.is_shutdown() ):
                    ...
                    if( self.vision_path.num_point == 0 ):
                        ...
                        relative_y = -1*_PATH_FORCE_Y_
                    elif( self.vision_path.x_point[0] < -30 ):
                        relative_y = _PATH_FORCE_Y_
                    else:
                        ok_y = True
                    ...
                    if( self.vision_path.num_point == 0):
                        ...
                        relative_x = _PATH_FORCE_X_
                    elif( self.vision_path.y_point[0] < -20 ):
                        relative_x = -1*_PATH_FORCE_X_
                    else:
                        ok_x = True
            ...
            if( mode == 1 ):
                ...
                if( diff_time > _PATH_FIND_TIME_ ):
                    self.control.publish_data( "FIND mode 1 time out")
                    mode = 2
                    start_time = rospy.get_rostime()
            elif( mode == 2 ):
                self.control.force_xy( 0.1 , -1.2 )
                if( diff_time > (_PATH_FIND_TIME_ * 2.0) + 2 ):
                    self.control.publish_data( "FIND mode 2 time out")
                    mode = 3
                    start_time = rospy.get_rostime() 
            elif( mode == 3 ):
                self.control.force_xy( 0.1 , 1.2 )
                if( diff_time > ( _PATH_FIND_TIME_ + 2 ) ):
...
#        self.control.absolute_yaw( self.first_yaw )
        ...
        while( not rospy.is_shutdown() ):
            ...
            self.vision.call_data()
            self.vision.echo_data()
            diff = zeabus_math.bound_radian( self.vision.rotation[0] - math.pi )
    ...
    def moving_on_path( self ): # status_mission = 3
        ...
        while( ( not rospy.is_shutdown() ) ):
            ...
            if( self.vision.x_point[ 0 ] < -20  ):
                force_y = _PATH_FORCE_Y_
            elif( self.vision.x_point[ 0 ] > 20 ):
                force_y = -1.0*_PATH_FORCE_Y_
            elif( self.vision.y_point[ 0 ] < -20 ):
                force_x = -1.0*_PATH_FORCE_X_
            elif( self.vision.y_point[ 0 ] > 20 ):
                force_x = _PATH_FORCE_X_
...
#        self.control.force_xy(0,0)
        ...
        start_time = rospy.get_rostime()
        diff_time = ( rospy.get_rostime() - start_time).to_sec()
        while( not rospy.is_shutdown() and diff_time < _PATH_LAST_TIME_ ):
            self.rate.sleep()
            self.control.force_xy( 1.8 , 0 )
            diff_time = ( rospy.get_rostime() - start_time).to_sec()
