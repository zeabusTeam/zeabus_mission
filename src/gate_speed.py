...
class Gate:
    ...
    def start_mission( self ): # state mission is 0
        ...
        self.control.absolute_z( _GATE_START_DEPTH_ )
        ...
        while( diff_time < _GATE_START_FORWARD_TIME_ and count < 3):
            self.rate.sleep()
            ...
            if( self.control.force_xy( 1.2 , 0 ) > _GATE_START_FORWARD_DISTANCE_ ):
        ...
        if( count == 3 ):
            ...
            self.control.force_xy( 0 , 1.5 * _GATE_START_SURVEY_DIRECTION_ , True )
            count = 0 

        # Next If you don't found picture and count == 3 I will survey
        start_time = rospy.get_rostime()
        diff_time = ( rospy.get_rostime() - start_time ).to_sec()  
        while( diff_time < _GATE_START_SURVEY_TIME_ and count < 3 ):
            self.rate.sleep()
            ...
            if( self.control.force_xy( 0 , 1.5 * _GATE_START_SURVEY_DIRECTION_ ) 
                    > _GATE_START_SURVEY_DISTANCE_ ):
                self.control.publish_data( "START Abort forward by distance")
                break
    ...
    def direct_search( self ):
        ...
        self.control.force_xy( 1.2 , 0 , True )
        while( ( diff_time < _GATE_FORWARD_ONLY_TIME_ ) and ( count < 3 ) ):
            self.rate.sleep()
            self.vision.call_data()
            self.vision.echo_data()
            ...
            if( self.control.force_xy( 1.2 , 0 ) < _GATE_FORWARD_ONLY_DISTANCE_ ):
